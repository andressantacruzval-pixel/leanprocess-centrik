...
export function RiskListCard({ params }: { params: Record<string, string> }) {
  ...
  if (!rows.length) {
    return (
      <div className="text-[12px] text-white/40 border border-white/10 rounded-xl px-3 py-2.5">
  ...
  return (
    <div className="space-y-1.5">
      {rows.map(({ risk, processName, level, executor, adequate }) => {
        ...
        return (
          <div key={risk.id} className={`rounded-xl border p-2.5 ${adequate ? 'border-white/10 bg-white/[0.03]' : 'border-red-500/25 bg-red-500/[0.06]'}`}>
            <div className="flex items-start gap-2">
              {adequate ? <ShieldCheck size={14} className="mt-0.5 shrink-0 text-emerald-400" /> : <ShieldAlert size={14} className="mt-0.5 shrink-0 text-red-400" />}
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[12.5px] font-semibold text-white">{risk.title}</span>
                  <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full text-white" style={{ background: hex }}>{level}</span>
                  <span className="text-[10px] text-white/40">{risk.category}</span>
                </div>
                <div className="text-[10.5px] text-white/40 mt-0.5">{processName} · Ejecutor: {executor}</div>
                <div className="flex items-center gap-2 mt-1">
                  <span className={`text-[10.5px] font-medium ${adequate ? 'text-emerald-300' : 'text-red-300'}`}>{adequate ? 'Con control adecuado' : 'Sin control adecuado'}</span>
                  {proc && (
                    <Link to={docPath('characterization', proc.id)} className="text-[10.5px] text-cyan-300 hover:underline inline-flex items-center gap-0.5">
      ...
      <Link to={HEAT_MAP_PATH} className="inline-flex items-center gap-1 text-[11px] text-white/50 hover:text-cyan-300 pt-0.5">
