...
// Caja de entrada: texto + dictado por voz + enviar/detener.
export function Composer({ isStreaming, onSend, onStop }: { isStreaming: boolean; onSend: (text: string) => void; onStop: () => void }) {
  ...
  const submit = useCallback(() => {
    ...
    onSend(t)
    setValue('')
  }, [value, isStreaming, onSend, dictado])
    ...
    <div className="border-t border-white/5 bg-[#0b111c] p-3">
      ...
      <div className="flex items-end gap-2">
        <textarea
          ...
          placeholder="Pregúntale al copiloto sobre tus procesos, riesgos, indicadores…"
          rows={1}
        ...
        {isStreaming ? (
          ...
          <button
            ...
            disabled={!value.trim()}
            title="Enviar"
            className="shrink-0 h-10 w-10 flex items-center justify-center rounded-xl bg-gradient-to-br from-cyan-500 to-blue-600 text-white disabled:opacity-30 disabled:cursor-not-allowed hover:opacity-90 transition-opacity"
          >
