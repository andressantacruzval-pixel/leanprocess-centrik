...
export function ConversationRail({ conversations, activeId, onNew, onSelect, onRename, onDelete }: Props) {
  ...
  return (
    <aside className="hidden md:flex w-64 shrink-0 flex-col border-r border-white/5 bg-[#0b111c]">
      <div className="p-3 space-y-2 border-b border-white/5">
        <button
          onClick={onNew}
          className="w-full inline-flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-[13px] font-medium hover:opacity-90 transition-opacity"
        >
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Buscar…"
            className="w-full pl-8 pr-2.5 py-1.5 rounded-lg bg-white/5 border border-white/10 text-[12px] text-white placeholder-white/25 focus:outline-none focus:border-cyan-500/40"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
        {filtered.length === 0 && (
          <p className="text-[11.5px] text-white/25 text-center px-3 py-6">
        ...
        {filtered.map((c) => {
          ...
          return (
            ...
            >
              <MessageSquare size={13} className={`shrink-0 ${isActive ? 'text-cyan-400' : 'text-white/30'}`} />
              {isEditing ? (
                <>
                  <input
                    value={draft}
                    ...
                    autoFocus
                    className="flex-1 min-w-0 bg-white/10 rounded px-1.5 py-0.5 text-[12.5px] text-white focus:outline-none"
...
function IconBtn({ children, onClick, title, danger }: { children: React.ReactNode; onClick: (e: React.MouseEvent) => void; title: string; danger?: boolean }) {
  return (
    <button title={title} onClick={onClick} className={`p-1 rounded text-white/40 hover:bg-white/10 ${danger ? 'hover:text-red-300' : 'hover:text-cyan-300'}`}>
      {children}
    </button>
  )
}
