...
const TIER_COLORS: Record<Achievement['tier'], { border: string; glow: string; label: string }> = {
  bronze:   { border: 'border-[#CD7F32]', glow: '',  label: 'Bronce' },
  silver:   { border: 'border-[#C0C0C0]', glow: '', label: 'Plata' },
  gold:     { border: 'border-[#FFD700]', glow: '',   label: 'Oro' },
  platinum: { border: 'border-[#E5E4E2]', glow: '', label: 'Platino' },
}
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2 rounded-lg bg-amber-50 px-4 py-2 ring-1 ring-amber-500">
          <Trophy className="h-6 w-6 text-amber-600" />
          <span className="text-2xl font-bold text-amber-700">{totalPoints}</span>
          <span className="text-sm text-amber-600">puntos</span>
        </div>
          ...
          <button
            ...
            className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${
              filter === cat.key
                ...
                : 'bg-gray-50 text-gray-400 hover:bg-gray-100 hover:text-gray-300'
            }`}
          >
            {cat.label}
          </button>
          ...
          return (
            <div
              ...
              className={`relative flex flex-col gap-3 rounded-lg border p-4 transition-all ${
                isUnlocked
                  ? `${tier.border} ${tier.glow} bg-gray-50`
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${
                      isUnlocked
                        ? `${TIER_TEXT[achievement.tier]} bg-gray-100`
              ...
              {isUnlocked ? (
                <div className="flex items-center justify-between">
                  ...
                  {onShare && (
                    <button
                      ...
                      className="flex items-center gap-1 rounded-md bg-primary-50 px-2 py-1 text-[10px] font-medium text-primary-600 transition-colors hover:bg-primary-100"
                    >
                ...
                <div className="mt-auto rounded-md bg-gray-50 px-2 py-1.5">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-gray-500">
                  ...
                  {progress[achievement.id] && (
                    <div className="mt-1 h-1 rounded-full bg-gray-50 overflow-hidden">
                      <div
