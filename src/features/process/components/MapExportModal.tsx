...
export function MapExportModal({ data, invRows, author, country, onClose }: {
  ...
  const seg = (on: boolean) => `flex-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition-colors ${on ? 'bg-primary-100 text-primary-700 border border-primary-300' : 'text-gray-500 border border-gray-200 hover:text-gray-800'}`
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/45 p-3" onClick={onClose}>
      <div className="w-full max-w-5xl max-h-[92vh] overflow-hidden rounded-lg border border-gray-200 bg-white flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
          <div>
            <h3 className="text-sm font-semibold text-gray-900">Exportar mapa de procesos</h3>
            <p className="text-[11px] text-gray-500">Vectorial y con tu marca — nunca sale cortado. La configuración se guarda.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-4 p-4">
          ...
          <div className="min-w-0">
            <div className="rounded-lg border border-gray-200 overflow-auto max-h-[60vh]" style={{ background: brand.theme === 'light' ? '#e9eff8' : '#0b1220' }}>
              <div className="p-3" dangerouslySetInnerHTML={{ __html: previewSvg }} />
            </div>
            <div className="flex flex-wrap gap-2 mt-3">
              <button onClick={doPng} disabled={!!busy} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-[12px] font-medium bg-primary-50 text-primary-700 border border-primary-300 hover:bg-primary-100 disabled:opacity-50">
                {busy === 'png' ? <Loader2 size={14} className="animate-spin" /> : <ImageIcon size={14} />} PNG
              </button>
              <button onClick={doSvg} disabled={!!busy} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-[12px] font-medium bg-primary-50 text-primary-700 border border-primary-300 hover:bg-primary-100 disabled:opacity-50">
                <FileCode size={14} /> SVG vectorial
              </button>
              <button onClick={doPdf} disabled={!!busy} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-[12px] font-medium bg-red-50 text-red-700 border border-red-300 hover:bg-red-100 disabled:opacity-50">
                {busy === 'pdf' ? <Loader2 size={14} className="animate-spin" /> : <FileText size={14} />} PDF: mapa + inventario
              </button>
            </div>
            <p className="text-[10px] text-gray-400 mt-1.5">SVG se edita en PowerPoint/Illustrator sin perder nitidez. El PDF incluye el mapa, los gráficos y la tabla del inventario.</p>
          </div>

          {/* Controls */}
          <div className="space-y-4">
            <div>
              <h4 className="text-[11px] font-semibold uppercase tracking-wide text-gray-500 mb-2">Combinaciones</h4>
              <div className="grid grid-cols-2 gap-1.5">
                {MAP_PRESETS.map((p) => (
                  <button key={p.id} onClick={() => st.applyPreset(p.id)}
                    className={`text-left rounded-lg border p-2 transition-colors ${st.preset === p.id ? 'border-primary-300 bg-primary-50' : 'border-gray-200 hover:border-gray-300'}`}>
                    <div className="flex gap-1 mb-1">
                      {[p.cEst, p.cPro, p.cApo, p.accent].map((c, i) => <span key={i} className="w-3.5 h-3.5 rounded-sm" style={{ background: c }} />)}
                    </div>
                    <span className="block text-[11px] font-semibold text-gray-800 leading-tight">{p.name}</span>
                    <span className="block text-[9px] text-gray-400">{p.tag}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              <h4 className="text-[11px] font-semibold uppercase tracking-wide text-gray-500 mb-2">Color por franja</h4>
              <div className="space-y-1.5">
                {colors.map((c) => (
                  <label key={c.key} className="flex items-center gap-2">
                    <input type="color" value={st[c.key]} onChange={(e) => st.setBrand({ [c.key]: e.target.value })} className="w-8 h-7 rounded-md border border-gray-200 bg-transparent cursor-pointer p-0.5" />
                    <span className="flex-1 min-w-0"><span className="block text-[11px] text-gray-800 leading-tight">{c.label}</span><span className="block text-[9px] text-gray-400">{c.sub}</span></span>
                    <span className="text-[9px] font-mono text-gray-400">{st[c.key]}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div>
              <h4 className="text-[11px] font-semibold uppercase tracking-wide text-gray-500 mb-2">Fondo</h4>
            ...
            <div>
              <h4 className="text-[11px] font-semibold uppercase tracking-wide text-gray-500 mb-2">Composición</h4>
              <div className="space-y-1.5">
                {toggles.map((t) => (
                  <button key={t.key} onClick={() => st.setBrand({ [t.key]: !st[t.key] })}
                    className="w-full flex items-center justify-between gap-2 text-left rounded-lg border border-gray-200 px-2.5 py-1.5 hover:border-gray-300">
                    <span className="text-[11px] text-gray-700">{t.label}</span>
                    <span className={`w-8 h-4 rounded-full relative transition-colors ${st[t.key] ? 'bg-primary-100' : 'bg-gray-200'}`}>
            ...
            <div>
              <h4 className="text-[11px] font-semibold uppercase tracking-wide text-gray-500 mb-2">Datos del entregable</h4>
              <div className="space-y-2">
                <label className="block">
                  <span className="block text-[10px] text-gray-500 mb-0.5">Alcance (opcional)</span>
                  <input value={st.scope} onChange={(e) => st.setMeta({ scope: e.target.value })} placeholder="Toda la organización"
                    className="w-full bg-gray-50 border border-gray-200 rounded-lg px-2.5 py-1.5 text-[12px] text-gray-800 outline-none placeholder-gray-400" />
                </label>
                <label className="block">
                  <span className="block text-[10px] text-gray-500 mb-0.5">Versión</span>
        ...
        <div className="px-4 py-2.5 border-t border-gray-200 flex items-center justify-between">
          <span className="text-[10px] text-gray-400 inline-flex items-center gap-1"><Download size={11} /> Los cambios se guardan automáticamente para la próxima vez.</span>
          <button onClick={onClose} className="px-3 py-1.5 rounded-lg text-[12px] text-gray-600 hover:text-gray-800 border border-gray-200">Cerrar</button>
        </div>
      </div>
    </div>
  )
