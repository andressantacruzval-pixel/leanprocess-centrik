...
export function AssetLifecycleModal({ asset, initialProcId, onClose }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[85] flex items-center justify-center bg-gray-900/45 p-4">
      <div className={`bg-white rounded-lg shadow-xl w-full ${mode === 'matrix' ? 'max-w-4xl' : 'max-w-2xl'} max-h-[88vh] flex flex-col border border-gray-200`}>
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 shrink-0">
          <div className="min-w-0">
            <div className="flex items-center gap-2"><Route size={15} className="text-primary-600" /><h3 className="text-sm font-semibold text-gray-900 truncate">Ciclo de vida · {liveAsset.name}</h3></div>
            <p className="text-[11px] text-gray-500 mt-0.5">Cómo viaja cada columna por los subprocesos y qué tratamiento recibe en cada uno.</p>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            <div className="flex rounded-lg border border-gray-200 overflow-hidden">
              <button onClick={() => setMode('stepper')} className={`inline-flex items-center gap-1 px-2 py-1.5 text-[10.5px] ${mode === 'stepper' ? 'bg-primary-100 text-primary-700' : 'text-gray-500 hover:bg-gray-50'}`}><ListTree size={12} /> Por subproceso</button>
              <button onClick={() => setMode('matrix')} className={`inline-flex items-center gap-1 px-2 py-1.5 text-[10.5px] ${mode === 'matrix' ? 'bg-primary-100 text-primary-700' : 'text-gray-500 hover:bg-gray-50'}`}><LayoutGrid size={12} /> Matriz</button>
            </div>
        ...
        {mode === 'matrix' ? (
          ...
          <p className="text-[12px] text-gray-500 p-8 text-center">Este activo no tiene recorrido registrado.</p>
        ) : (
          <>
            ...
            <div className="px-5 py-2.5 border-b border-gray-100 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[14px] font-semibold text-gray-900">{stage.name}</span>
              </div>
            </div>
        ...
        {mode === 'stepper' && stages.length > 0 && (
          <div className="flex items-center gap-2 px-4 py-2.5 border-t border-gray-100 shrink-0">
            <button onClick={() => setIdx((i) => Math.max(0, i - 1))} disabled={safeIdx === 0}
              className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-gray-50 border border-gray-200 text-[11px] text-gray-700 hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed"><ChevronLeft size={14} /> Anterior</button>
              ...
              <div className="flex items-center gap-1.5 justify-center min-w-min px-1">
                {stages.map((s, i) => (
                  <button key={s.procId} onClick={() => setIdx(i)} title={s.name}
                    className={`px-2 py-1 rounded-lg text-[10.5px] whitespace-nowrap border transition-colors ${i === safeIdx ? 'bg-primary-100 text-primary-700 border-primary-300' : 'text-gray-500 border-gray-100 hover:bg-gray-50'}`}>
            ...
            <button onClick={() => setIdx((i) => Math.min(stages.length - 1, i + 1))} disabled={safeIdx >= stages.length - 1}
              className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-gray-50 border border-gray-200 text-[11px] text-gray-700 hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed">Siguiente <ChevronRight size={14} /></button>
          </div>
        )}
      </div>
    </div>,
