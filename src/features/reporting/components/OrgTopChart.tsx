...
export function OrgTopChart({ title, sub, items, org, color = '#06b6d4' }: {
  ...
  const toggle = (
    <div className="flex flex-wrap gap-0.5 rounded-lg border border-white/10 bg-white/[0.03] p-0.5">
      {tabs.map((t) => (
        <button key={t.k} onClick={() => setLvl(t.k)}
          className={`px-2 py-0.5 rounded text-[10px] transition-colors ${lvl === t.k ? 'bg-cyan-500/20 text-cyan-300' : 'text-white/40 hover:text-white/70'}`}>
          {t.label}
        </button>
      ))}
    </div>
