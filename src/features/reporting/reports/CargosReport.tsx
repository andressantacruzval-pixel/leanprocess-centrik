...
import { useCargoData, CARGO_CATALOG, type CargoAgg } from '@/features/cargos/cargoData'
...
// Reporte por Cargo: cuántos procesos y actividades tiene cada cargo, cruzado
// con el análisis de valor (tiempo VA/NVA/NVABN por cargo). Detecta cargos que
// aún no están en el catálogo y permite darlos de alta en un clic.

const min = (n: number) => `${Math.round(n)}`
...
export function CargosReport() {
  ...
  const totalVa = conActividad.reduce((s, c) => s + c.vaMin, 0)
  const totalMed = conActividad.reduce((s, c) => s + c.totalMin, 0)
  ...
  const byNva = [...conActividad].sort((a, b) => b.nvaMin - a.nvaMin).slice(0, 8)
    .filter((c) => c.nvaMin > 0).map<Datum>((c) => ({ label: c.cargo, value: Math.round(c.nvaMin), color: '#f87171' }))
  ...
  return (
    <Dashboard>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card title="Desperdicio por cargo" sub="Tiempo NVA min/mes (top 8)."><HBars data={byNva} /></Card>
      ...
      <div className="space-y-2">
        ...
        {byNva.length > 0 && <Insight tone="crit">«{byNva[0].label}» concentra {min(byNva[0].value)} min/mes en actividades sin valor (NVA). Candidato a rediseñar o redistribuir carga.</Insight>}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span className="ml-auto text-[11px] text-white/30">{shown.length} de {cargos.length} cargos</span>
      </div>
      ...
      <TableWrap minWidth={1100}>
        <thead>
          <tr className="bg-white/[0.03] border-b border-white/5">
            ...
            <Th>VA min/mes</Th><Th>NVA min/mes</Th><Th>NVABN min/mes</Th><Th>% Valor</Th>
          </tr>
        </thead>
        <tbody>
          {shown.map((c) => {
            const pv = c.totalMin > 0 ? Math.round(c.vaMin / c.totalMin * 100) : null
            ...
            return [
              <tr key={c.key} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                ...
                <Td className="tabular-nums"><span style={{ color: CLASSIFICATION_COLORS.VA.hex }}>{c.vaMin ? min(c.vaMin) : '-'}</span></Td>
                <Td className="tabular-nums"><span style={{ color: CLASSIFICATION_COLORS.NVA.hex }}>{c.nvaMin ? min(c.nvaMin) : '-'}</span></Td>
                <Td className="tabular-nums"><span style={{ color: CLASSIFICATION_COLORS.NVABN.hex }}>{c.nvabnMin ? min(c.nvabnMin) : '-'}</span></Td>
...
function CargoDetail({ cargo }: { cargo: CargoAgg }) {
  return (
    <div className="rounded-lg border border-white/5 bg-white/[0.02] p-2">
      <div className="text-[10px] uppercase tracking-wide text-white/35 mb-1.5">Actividades de «{cargo.cargo}» ({cargo.activities.length})</div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1">
        {cargo.activities.map((a, i) => {
          const c = a.classification ? CLASSIFICATION_COLORS[a.classification] : null
          return (
            <div key={i} className="flex items-center gap-2 text-[11px]">
              {c ? <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: c.hex }} /> : <span className="w-1.5 h-1.5 rounded-full shrink-0 bg-white/20" />}
              <span className="text-white/75 truncate flex-1">{a.activityName}</span>
              <span className="text-white/35 truncate max-w-[160px]">{a.processName}</span>
              {a.monthlyMinutes > 0 && <span className="text-white/45 tabular-nums w-16 text-right">{min(a.monthlyMinutes)} m/mes</span>}
            </div>
          )
        })}
      </div>
