  ...
  return (
    ...
    >
      <div
        ...
        className={`w-full ${ANCHOS[size]} max-h-[85vh] flex flex-col bg-[#0d1424] border border-white/10 rounded-2xl shadow-2xl overflow-hidden`}
      >
        {(title || icon) && (
          <div className="shrink-0 flex items-start gap-3 px-5 py-4 border-b border-white/5">
            ...
            <button
              onClick={onClose}
              aria-label="Cerrar"
              className="shrink-0 -m-2 p-2 rounded-lg text-white/30 hover:text-white/70 hover:bg-white/5 transition-colors"
            >
              <X size={16} />
            </button>
          </div>
        ...
        {footer && (
          <div className="shrink-0 flex flex-wrap items-center justify-end gap-3 px-5 py-4 border-t border-white/5">
            {footer}
          </div>
        )}
      </div>
